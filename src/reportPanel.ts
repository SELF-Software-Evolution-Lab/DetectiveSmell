...
private async _update(proyecto: string, globalReport: GlobalReport) {
    ...
    private _getHtmlForWebview(webview: vscode.Webview, proyecto: string, globalReport: GlobalReport) {
        ...
        const projectName = proyecto?.split("\\").pop();
                ...
                <body>
                    ...
                    ${Object.entries(globalReport).map(([category, rules]) => `
                        <div class="card">
                            <h3>${category}</h3>
                            ${rules.length === 0 ? 
                            `<h4>No hay reglas violadas en esta capa</h4>` :
                            rules.map(rule => `
                                <div class="rule">
                                    <div class="flexbox" onclick="this.nextElementSibling.classList.toggle('show')">
                                        <div class="flex">
                                            <img src="${circleUri}-${rule.severity.toLowerCase()}.png"></img>
                                            <h3>${rule.name} en el archivo <span><b>${rule.absolutePath}</b></span></h3>
                                        </div>
                                    </div>
                                    <div class="rule-detail">
                                        <div class="contbtn">
                                            <button class="btn" onclick="abrirArchivo('${rule.path}',${rule.line})">
                                                <span>Ir al archivo 
